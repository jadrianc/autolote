...
class PDF_Boleta extends FPDF
{
    ...
    public function WaterMark()
    {
        ...
        if(strlen($this->data["ONIUSER"]) <= 5){
            ...
            $this->RotatedText(20, 210, $this->data["ONIUSER"], 25);
        } else {
            $this->RotatedText(20, 208, $this->data["ONIUSER"], 25);
        }
    ...
    public function Header()
    {
        ...
        $this->Image($logo, 12, 8, 15);
        $margenTabla = 100;
        $this->SetTextColor(0, 0, 0);
        $this->SetFont('Arial', '', 9);
        $fecha = $this->actual_date();
        $this->Text(70, 17, utf8_decode('Boleta de control esquelas de infraccion impuestas por cada elemento. Uso exclusivo del elemento policial'));
        $this->Text(70, 22, utf8_decode('Nombre: '.$this->data['NOMBRE']));
        $this->Text(110, 22, utf8_decode('ONI: '.$this->data['ONIUSER']));
        $this->Text(140, 22, utf8_decode('Unidad: '.$this->data['UNIDAD']));
        //$this->Text(70, 21, utf8_decode('UNIDAD: '.$this->data['UNIDAD']));
       // $this->Text(70, 25, utf8_decode('PERIODO: DESDE '.$this->data['DESDE']. " HASTA ".$this->data['HASTA']. " USUARIO: ".$this->data['USUARIO']));
       // $this->Text(15, 49, 'DEPARTAMENTO DE SUMINISTROS');
        ...
        $altoEncab = 5;
        /*
        $this->Cell(8,$altoEncab,'n',1,0,'C');
        $this->Cell(20,$altoEncab,'SERIE',1,0,'C');
        $this->Cell(20,$altoEncab,utf8_decode('COD'),1,0,'C');
        $this->Cell(20,$altoEncab,'PLACA',1,0,'C');
        $this->Cell(60,$altoEncab,'NOMBRE DEL INFRACTOR',1,0,'C');
        $this->Cell(60,$altoEncab,'LUGAR INFRACCION',1,0,'C');
        $this->Cell(25,$altoEncab,'FECHA',1,0,'C');
        $this->Cell(25,$altoEncab,'CLASE Y NUM LICENCIA ',1,0,'C');
        $this->Cell(20,$altoEncab,'ONI RECIBE',1,0,'C');
        $this->SetX(8);
        $this->SetY(35);
        */
      /*  $this->MultiCell(8,10,'n',1,'C');
        $this->SetXY(16, 30);
        
        $this->MultiCell(20,10,'SERIE',1,'C');
        $this->SetXY(36, 30);
        $this->MultiCell(20,10,'COD',1,'C');
        $this->SetXY(56, 30);
        $this->MultiCell(20,10,'PLACA',1,'C');
        $this->SetXY(76, 30);
        $this->MultiCell(60,10,'NOMBRE DEL INFRACTOR',1,'C');
        $this->SetXY(136, 30);
        $this->MultiCell(60,10,'LUGAR INFRACCION',1,'C');
        $this->SetXY(196, 30);
        $this->MultiCell(25,10,'FECHA',1,'C');
        $this->SetXY(221, 30);
        $this->MultiCell(25,5,'CLASE Y NUM LICENCIA',1,'C');
        $this->SetXY(246, 30);
        $this->MultiCell(20,5,'ONI RECIBE',1,'C');
        $this->SetX(8);
        */
...
{  
    ...
    public function Row($data,$colorear)
    {
        ...
        for ($i=0;$i<count($data);$i++) {
            ...
            $this->Rect($x, $y, $w, $h+1);
            //Print the text
            $this->MultiCell($w, 4, $data[$i], 0, 'C', $colorear);
            //Put the position to the right of the cell
            $this->SetXY($x+$w, $y);
        }
        //Go to the next line
