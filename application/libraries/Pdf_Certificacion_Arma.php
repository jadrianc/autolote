...
class Pdf_Certificacion_Arma extends FPDF
{
    public $widths;
    public $aligns;
    public $title;
    ...
    public function Header()
    {
        ...
        $this->Text(15, 44, 'DIVISION DE LOGISTICA');
        ...
        $this->MultiCell(170,7,'ESPECIFICACIONES DEL ARMA: Tipo: '.$this->data['TIPO'].' '.$this->data['CALIBRE'].' '.$this->data['MARCA'].' MOD '.$this->data['MODELO'],'B','L');
        $this->SetFont('Arial', '', 9);
        $this->SetY(80);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Serie:  '.$this->data['SERIE']),0,'L');
        $this->SetY(86);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Marca:  '.$this->data['MARCA']),0,'L');
        $this->SetY(92);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Modelo:  '.$this->data['MODELO']),0,'L');
        $this->SetY(98);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('N° Factura:  '.$this->data['NUMERO_FACTURA']),0,'L');
        $this->SetY(104);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('ID Caja:  '.$this->data['PACKI']),0,'L');
        $this->SetY(110);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Póliza:  '.$this->data['NUMERO_POLIZA']),0,'L');
        $this->SetY(116);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('CAF:  '.$this->data['PACKI']),0,'L');
        $this->SetY(122);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Fecha Adquisición:  '.$this->data['FECHAADQUI']),0,'L');
        $this->SetY(128);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Adquisición:  '.$this->data['ADQUI']),0,'L');
        $this->SetY(134);
        $this->SetX($margenTabla);
        $this->MultiCell(100,10,utf8_decode('Proveedor:  '.$this->data['PROVEEDOR']),0,'L');

        $this->Text(15, 160, 'Por medio de la presente de hace constar que el arma con las especificaciones arriba detalladas pertenece a');
        $this->Text(15, 167, 'la Policia Nacional Civil.');
        $this->Text(15, 178, 'Y para los usos legales que estime, se extiende la presente constancia de propiedad');
        $this->Text(15, 200, 'Atentamente,');
        $this->Text(15, 257, 'MA / '.$this->data['ONI_USER']);
        $this->Text(80, 240, utf8_decode($this->data['CARGO_JEFE'].' '.$this->data['NOMBRE_JEFE']));
        $this->Text(95, 244, 'Jefe/a del Departamento de Suministros.');
...
{  
    ...
    {
        return new Pdf_Certificacion_Arma();
    }
}
