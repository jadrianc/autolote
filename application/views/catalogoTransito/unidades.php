              ...
              <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form id="marcaForm" autocomplete="off" name="marcaForm" class="form-horizontal">
                              ...
                              <input type="hidden" class="form-control" id="id_unidad" name="id_unidad" value="0" 
                              placeholder="Ingrese un Codigo a la Marca" value="" maxlength="5" required>
                          
                        <div class="form-group">
                          <label for="name" class="col-sm-3 control-label">Unidad</label>
                          <div class="col-sm-12">
                              <input type="text" class="form-control text-uppercase" id="unidad" name="unidad" 
                              placeholder="Ingrese Nombre de Unidad" value="" maxlength="60" required>
                          </div>
                        </div>
                        <div class="form-group">
                            <label>ONI Jefe</label>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control text-uppercase" id="categoria" name="categoria" required readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="form-group">
                            <label>Nombre Cargo</label>
                            <input type="text" class="form-control text-uppercase" id="cargo" name="cargo" required>
                            </div>
                        </div>
                        </div>
   ...
   $('#msj').hide();
    ...
    $('#marca_list tbody').on('click', '.btn-edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
          $.ajax({ 
                url: "unidadesTransito/getById/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    $('#marca_list tbody').on('click', '.btn-delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
          Swal.fire({
          title: 'Esta seguro de borrar la unidad: '+ data['NOMBRE'] + '?',
          ...
          if (result.value) {
            $.ajax({ 
                url: "unidadesTransito/delete/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    function setForm(data){
      console.log(data);
      $("#id_unidad").prop('readonly', true);
      $('#id_unidad').val(data['ID_FILA']);
      $('#unidad').val(data['NOMBRE']);
      $('#oni_asignado').val(data['ONI_JEFE']);
      $('#categoria').val(data['CATEGORIA']);
      $('#cargo').val(data['NOMBRE_CARGO']);
    }
    ...
    $('#create-new').click(function () {
         ...
         $('#btn-save').val("create");
         $('#marcaForm').trigger("reset");
         $('#formModal').html("Agregar Nueva Unidad");
         $('#ajax-modal').modal('show');
      });
        ...
        $("#marcaForm").validate({
         ...
         submitHandler: function (form) {
            ...
            if(actionType == 'create'){
              ...
              urlRequest = 'unidadesTransito/store';
            } else if(actionType == 'update') {
              urlRequest = 'unidadesTransito/update';
            }
