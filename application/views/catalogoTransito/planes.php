                    ...
                    <form id="marcaForm" autocomplete="off" name="marcaForm" class="form-horizontal">
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-sm-12">
                              ...
                              placeholder="Ingrese nuevo plan" value="" maxlength="60" required>
                          </div>
                        </div>
                        </div>
  ...
  $(document).ready(function() {
    ...
    $('#marca_list tbody').on('click', '.btn-edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
          $.ajax({ 
                url: "planesTransito/getById/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    $('#marca_list tbody').on('click', '.btn-delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
          ...
          if (result.value) {
            $.ajax({ 
                url: "planesTransito/delete/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
                ...
                success: function (res) {
                  ...
                  Swal.fire(
                    ...
                    Swal.fire({
                      ...
                      text: 'Este dato esta siendo usado en otro lados',
                      footer: 'TRANSITO'
                    })
                  }
                },
    ...
    $('#create-new').click(function () {
         ...
         $('#btn-save').val("create");
         $('#marcaForm').trigger("reset");
         $('#formModal').html("Agregar Nuevo Plan");
         $('#ajax-modal').modal('show');
      });
        ...
        $("#marcaForm").validate({
         ...
         submitHandler: function (form) {
            ...
            if(actionType == 'create'){
              urlRequest = 'planesTransito/store';
            } else if(actionType == 'update') {
              urlRequest = 'planesTransito/update';
            }
