                  ...
                  <div class="modal-body">
                    <form id="marcaForm" autocomplete="off" name="marcaForm" class="form-horizontal">
                        ...
                        <div class="form-group">
                          <label for="name" class="col-sm-3 control-label">Tipo Placa</label>
                          <div class="col-sm-12">
                              <input type="text" class="form-control text-uppercase" id="placa" name="placa" 
                              placeholder="Ingrese Tipo de Placa" value="" maxlength="60" required>
                          </div>
                        </div>
                        <div class="form-group">
  ...
  $(document).ready(function() {
    ...
    $('#marca_list tbody').on('click', '.btn-edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
          $.ajax({ 
                url: "TipoPlacas/getById/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
                ...
                success: function (res) {
                  if(res.success == true){
                    ...
                    $('#formModal').html("Editar Tipo Placa");
                    $('#btn-save').val("update");
                    setForm(res.data);
    ...
    $('#marca_list tbody').on('click', '.btn-delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
          ...
          if (result.value) {
            $.ajax({ 
                url: "TipoPlacas/delete/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    $('#create-new').click(function () {
         ...
         $('#btn-save').val("create");
         $('#marcaForm').trigger("reset");
         $('#formModal').html("Agregar Tipo Placa");
         $('#ajax-modal').modal('show');
      });
        ...
        $("#marcaForm").validate({
         ...
         submitHandler: function (form) {
            ...
            if(actionType == 'create'){
              urlRequest = 'TipoPlacas/store';
            } else if(actionType == 'update') {
              urlRequest = 'TipoPlacas/update';
            }
