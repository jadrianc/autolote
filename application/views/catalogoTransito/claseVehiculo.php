                  ...
                  <div class="modal-body">
                    <form id="marcaForm" autocomplete="off" name="marcaForm" class="form-horizontal">
                        ...
                        <div class="form-group">
                          <label for="name" class="col-sm-3 control-label">Clase Vehiculo</label>
                          <div class="col-sm-12">
                              <input type="text" class="form-control text-uppercase" id="clase" name="clase" 
                              placeholder="Ingrese una Clase de Vehiculo" value="" maxlength="60" required>
                          </div>
                        </div>
                        </div>
  ...
  $(document).ready(function() {
    ...
    $('#marca_list tbody').on('click', '.btn-edit', function () {
        var data = table.row( $(this).parents('tr') ).data();
          $.ajax({ 
                url: "claseVehiculo/getById/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    $('#marca_list tbody').on('click', '.btn-delete', function () {
        var data = table.row( $(this).parents('tr') ).data();
          ...
          if (result.value) {
            $.ajax({ 
                url: "claseVehiculo/delete/"+data['ID_FILA'],
                type: "GET",
                beforeSend: function (request) {
     request.setRequestHeader("Authorization",  <?php echo '"'.$token.'"'; ?>);
                },
    ...
    $('#create-new').click(function () {
         ...
         $('#btn-save').val("create");
         $('#marcaForm').trigger("reset");
         $('#formModal').html("Agregar Nueva Clase");
         $('#ajax-modal').modal('show');
      });
        ...
        $("#marcaForm").validate({
         ...
         submitHandler: function (form) {
            ...
            if(actionType == 'create'){
              urlRequest = 'claseVehiculo/store';
            } else if(actionType == 'update') {
              urlRequest = 'claseVehiculo/update';
            }
