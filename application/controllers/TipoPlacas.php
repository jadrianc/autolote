...
class TipoPlacas extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->tableName = 'TTO_CE_TIPOPLACAS';
    ...
    public function index()
    {
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isValidUser($is_valid_token['data']->user_name,$roles = array(1))) {
            ...
            $this->load->view('catalogoTransito/tipoPlacas', $data);
    ...
    public function store()
    {
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isAdminUser($is_valid_token['data']->user_name)){
            ...
            if ($this->input->post()) {
                ...
                $placa = xss_clean(strtoupper($this->input->post('placa'))); 
                $descripcion = xss_clean(strtoupper($this->input->post('descripcion')));    
                $newRecord = array('TIPO' => $placa, 'DESCRIPCION' => $descripcion);    
               // $this->form_validation->set_rules('id_marca', 'Codigo Marca', 'trim|required|numeric|max_length[10]');
                $this->form_validation->set_rules('placa', 'placa', 'trim|required|max_length[5]');   
                $this->form_validation->set_rules('descripcion', 'descripcion', 'trim|required|max_length[25]'); 
                //$reg = $this->TransitoModel->getId($this->tableName,'DESCRIPCION',$clase);
             
                    if ($this->form_validation->run() == true) {
                        $id = $this->TransitoModel->create($this->tableName,$newRecord);
                        $nuevoRegistro = $this->TransitoModel->getId($this->tableName,'TIPO',$placa);
                        $status = true;
    ...
    public function update()
    {
       ...
       if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isAdminUser($is_valid_token['data']->user_name)){
            ...
            if ($this->input->post()) {
                ...
                $placa = xss_clean(strtoupper($this->input->post('placa')));
                $descripcion = xss_clean(strtoupper($this->input->post('descripcion')));
                $data = array( 'ID_FILA' => $id_clase , 'TIPO' => $placa, 'DESCRIPCION' => $descripcion);
                $this->form_validation->set_rules('id_clase', 'Codigo Marca', 'trim|required|numeric|max_length[10]');
                $this->form_validation->set_rules('placa', 'placa', 'trim|required|max_length[5]');
                $this->form_validation->set_rules('descripcion', 'descripcion', 'trim|required|max_length[30]');
