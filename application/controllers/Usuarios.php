...
class Usuarios extends CI_Controller
{
    ...
    public function cambiarPassword()
    {
       ...
       if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isAdminUser($is_valid_token['data']->user_name)){
            ...
            $pass =  getHashedPassword($this->input->post('pass'));

            $this->form_validation->set_rules('id_usuario', 'ID Usuario', 'trim|required|numeric|max_length[10]');
            $this->form_validation->set_rules('pass', 'Password', 'trim|required|max_length[90]');

            if ($this->form_validation->run() == true) {
                $validate = $this->Usuarios_model->getById($id_usuario);
                if($validate)
                {
                    ...
                    $newRecord = array('ID_USUARIO' => $id_usuario, 'PASS' => $pass, 'ESTADO' => 'P');
                    $this->Usuarios_model->update($newRecord);
                }
            }
