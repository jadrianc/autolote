...
class TipoLicencia extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->tableName = 'TTO_CE_TIPOSLIC';
    ...
    public function index()
    {
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isValidUser($is_valid_token['data']->user_name,$roles = array(1))) {
            ...
            $this->load->view('catalogoTransito/tipoLicencia', $data);
    ...
    public function store()
    {
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isAdminUser($is_valid_token['data']->user_name)){
            ...
            if ($this->input->post()) {
                ...
                $licencia = xss_clean(strtoupper($this->input->post('licencia'))); 
                $descripcion = xss_clean(strtoupper($this->input->post('descripcion')));    
                $newRecord = array('CODIGO' => $licencia, 'DESCRIPCION' => $descripcion);    
               // $this->form_validation->set_rules('id_marca', 'Codigo Marca', 'trim|required|numeric|max_length[10]');
                $this->form_validation->set_rules('licencia', 'licencia', 'trim|required|max_length[5]');   
                ...
                //$reg = $this->TransitoModel->getId($this->tableName,'DESCRIPCION',$clase);
                    ...
                    if ($this->form_validation->run() == true) {
                        ...
                        $nuevoRegistro = $this->TransitoModel->getId($this->tableName,'CODIGO',$licencia);
                        $status = true;
    ...
    public function update()
    {
       ...
       if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isAdminUser($is_valid_token['data']->user_name)){
            ...
            if ($this->input->post()) {
                ...
                $licencia = xss_clean(strtoupper($this->input->post('licencia')));
                $descripcion = xss_clean(strtoupper($this->input->post('descripcion')));
                $data = array( 'ID_FILA' => $id_clase , 'CODIGO' => $licencia, 'DESCRIPCION' => $descripcion);
