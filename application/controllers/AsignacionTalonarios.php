...
class AsignacionTalonarios extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isValidUser($is_valid_token['data']->user_name,$roles = array(1,2))) {
            ...
            $data['title'] = 'Asignacion de Talonarios'; // Capitalize the first letter
            ...
            $this->load->view('asignacionTalonarios/index', $data);
    ...
    public function reporte(){
        ...
        if (!empty($is_valid_token) and $is_valid_token['status'] === true && $this->validateuser_library->isValidUser($is_valid_token['data']->user_name, $roles = array(1,2))){
            ...
            if($data->busqueda == 'fecha'){
                $fecha = $this->convertDateOracle($data->fecha);
                $res = $this->TransitoModel->reporte($fecha, "FECHA_ENTREGA", $data->oniUsuario, $rol);
                
           

            }else if($data->busqueda == 'num_talonario'){
                $final = $data->final;
                $res = $this->TransitoModel->reporte($final, "SERIE_INICIAL", $data->oniUsuario, $rol);
                
               
            }else{
                $oni = strtoupper($data->oni);
                $res = $this->TransitoModel->reporte($oni, "ONI_RECIBE", $data->oniUsuario, $rol);
                
                
            }

            if($rol){
                $arr = array('data' => $res, 'rol' => true); 
            }else{
                $arr = array('data' => $res, 'rol' => false);
